function [K, F, u] = processamento(N_el, n_nos, x, K, F, alfa_a, beta_a, gama_a, alfa_b, beta_b, gama_b, k_el, c_el, b_el, f_el);
  ...
  % Condição de contorno de Newmann ou mista em x = a
    ...
    K(1, 1) = K(1, 1) - k_el(1, 1)*beta_a/alfa_a;
    F(1) = F(1) - k_el(1, 1)*gama_a/alfa_a;
  endif
  ...
  % Condição de contorno de Newmann ou mista em x = b
    ...
    K(n_nos, n_nos) = K(n_nos, n_nos) + k_el(2, N_el)*beta_b/alfa_b;
    F(n_nos) = F(n_nos) + k_el(2, N_el)*gama_b/alfa_b;
  endif
...
function K = K_el(x_el, k_el, c_el, b_el)
  ...
  function y = c_int(c_el, x_el, A)
    ...
    c1 = c_el(1);
    c2 = c_el(2);
    y = (1/h^3)*( ( (x2^3/3)*(-c1 + c2) + (x2^2/2)*(c1*x2 - c2*x1 + A*(c1 - c2)) + x2*A*(-c1*x2 + c2*x1) ) - 
                  ( (x1^3/3)*(-c1 + c2) + (x1^2/2)*(c1*x2 - c2*x1 + A*(c1 - c2)) + x1*A*(-c1*x2 + c2*x1) ) );
  endfunction
  ...
  function y = b_int(b_el, x_el, A, B)
    ...
    b1 = b_el(1);
    b2 = b_el(2);
    y = (1/h^3)*( ( (x2^4/4)*(-b1 + b2) + (x2^3/3)*(b1*x2 - b2*x1 + (A+B)*(b1 - b2)) + (x2^2/2)*((A+B)*(-b1*x2 + b2*x1) + A*B*(-b1 + b2)) + x2*A*B*(b1*x2 - b2*x1) ) - 
                  ( (x1^4/4)*(-b1 + b2) + (x1^3/3)*(b1*x2 - b2*x1 + (A+B)*(b1 - b2)) + (x1^2/2)*((A+B)*(-b1*x2 + b2*x1) + A*B*(-b1 + b2)) + x1*A*B*(b1*x2 - b2*x1) ) );
  endfunction
endfunction
...
function F = F_el(x_el, f_el)
  ...
  function y = f_int(f_el, x_el, A)
    ...
    f1 = f_el(1);
    f2 = f_el(2);
    y = (1/h^2)*( ( (x2^3/3)*(-f1 + f2) + (x2^2/2)*(f1*x2 - f2*x1 + A*(f1 - f2)) + x2*A*(-f1*x2 + f2*x1) ) - 
                  ( (x1^3/3)*(-f1 + f2) + (x1^2/2)*(f1*x2 - f2*x1 + A*(f1 - f2)) + x1*A*(-f1*x2 + f2*x1) ) );
  endfunction
endfunction
